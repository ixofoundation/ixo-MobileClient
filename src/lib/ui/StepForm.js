    ...
    const steps = useMemo(
        () => {
            ...
            return steps.slice(
                ...
                MAX_STEP_COUNT + current,
            )
        }, 
        [current, total],
    )
    return <View style={stepInfoStyle.root}>
        {
            steps.map((i, k) => {
                ...
                return <Fragment key={i}>
                    ...
                    <View key={'step-' + i} style={s.container}>
                        {i < current && <Icon name='check' 
                            width={15} height={15} fill='#FFFFFF'/>}   
                        {i >= current && 
...
const stepItemStyle = (status) => StyleSheet.create({
    container: {
